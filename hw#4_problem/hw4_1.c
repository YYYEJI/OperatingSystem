/* 
	...
	Example)
		...
		$ vi ../test2.txt	t						// the user tries to create a file 'test2.txt' in the parent directory
		argc = 2
		argv[0] = vi
	...
	while(1){
		...
		if(argc > 0){
			...
			if(pid == 0){          // child process
				execvp(argv[0], argv);
                // fork의 return 값이 0인 경우는 error가 아니라 child의 경우입니다. error 메세지를 띄울 필요가 없습니다. 
                // exec()가 실패한 경우 error 메세지를 출력해주세요.
				...
				// status 변수를 사용하지 않아서 NULL을 넣으주면 될 것 같습니다. 
                int status;
				wait(&status);
			}
		}
...
//
	...
	/*
	int in_quote = 0;
    // 같은 기능을 하는 변수가 두개이며 quote_cnt는 불필요합니다.
    int quote_cnt = 0;
	...
	*/
    ...
    for (int i = 0; i<len; ++i) {
        if (cmd[i] == '\"' || cmd[i] == '\\') {  // if cmd[i] is a quote ('\"' or '\')
            ++i;  // increase i to bypass the quote
            argv[argc] = (cmd + i);  // set argv[argc] to (cmd + i) to store the starting address of a new argument into the argument array

            while (cmd[i] != '\0' && cmd[i] != '\"') {  // increase i until find the null character ('\0') or the closing quote
                if (cmd[i] == '/') {
                    argv[argc] = (cmd + i + 1);  // if cmd[i] is '/', set argv[argc] to (cmd + i + 1) to discard directory names
                }
                ++i;
            }
            ...
            argc++;  // increase argc
            if (cmd[i] == '\0') {
                --i;  // if cmd[i] is the null character, decrease i to make the condition of the for-loop fail
            } else {
                cmd[i] = '\0';  // otherwise, set cmd[i] to zero to make the current argument a string
            }

            if (strcmp(argv[argc - 1], "..") == 0) {  // if the new argument is "..", replace it by "."
                argv[argc - 1] = ".";
            }
        } else if (!isspace(cmd[i])) {  // if cmd[i] is not a whitespace
            argv[argc] = (cmd + i);  // set argv[argc] to (cmd + i) to store the starting address of a new argument into the argument array

            while (cmd[i] != '\0' && !isspace(cmd[i])) {  // increase i until find the null character ('\0') or a whitespace
                if (cmd[i] == '/') {
                    argv[argc] = (cmd + i + 1);  // if cmd[i] is '/', set argv[argc] to (cmd + i + 1) to discard directory names
                }
                ++i;
