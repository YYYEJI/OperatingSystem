/*
	...
	A consumer thread deletes a text message from the buffer and consumes it
...
void* Producer(void *arg)
{
	...
	while(*param->p_cont){
		...
		strcpy(mesg, message_array[rand_index]);
		...
		while (param->p_sb->count == BUFFER_SIZE) {
