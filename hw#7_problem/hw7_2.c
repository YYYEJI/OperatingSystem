...
// TO DO: complete this function following instructions
{
    ...
    pt->no_page = (pt->process_size + (pt->page_size - 1)) / pt->page_size;
	...
	pt->frame = (int*)malloc(pt->no_page * sizeof(int));
    if (pt->frame == NULL) {
        printf("Error: Failed to allocate memory for the page table.\n");
        exit(-1);
    }
	...
	memset(pt->frame, 0, pt->no_page * sizeof(int));
...
// load page table from a file
{
    ...
    fscanf(fp, "%d", &(pt->pagenum_bits));
    fscanf(fp, "%d", &(pt->offset_bits));
    fscanf(fp, "%d", &(pt->process_size));


    // Calculate derived values

	pt->page_size = 1 << pt->offset_bits;
    pt->no_page = (pt->process_size + (pt->page_size - 1)) / pt->page_size;
    pt->offset_mask = MakeMask(pt->offset_bits);
    ...
    pt->frame = (int *)malloc(pt->no_page * sizeof(int));
    if (pt->frame == NULL) {
        printf("Failed to allocate memory for frame array\n");
        fclose(fp);
        return FALSE;
    }

    // Read frame numbers from the file
    for (int p = 0; p < pt->no_page; p++) {
        fscanf(fp, "%d", &(pt->frame[p]));
    }
...
// Translate a logical address into physical address
{
	...
	if (logical >= pt->process_size) {
		printf("Error: Invalid logical address %d\n", logical);
		exit(-1);
	}

	int page_no = (logical >> pt->offset_bits) & ((1 << pt->pagenum_bits) - 1);
	int offset = logical & pt->offset_mask;
	int frame_no = pt->frame[page_no];
	physical = (frame_no << pt->offset_bits) | offset;


	// TO DO: for debug, display the logical address, page number, offset, frame number, and physical address
	printf("[DEBUG] logical = 0x%04x, page_no = 0x%04x, offset = 0x%04x, frame_no = 0x%04x, physical = 0x%04x\n", logical, page_no, offset, frame_no, physical);
	// printf("[DEBUG] logical = 0x%04x, page_no = 0x%04x, offset = 0x%04x, frame_no = 0x%04x, physical = 0x%05x\n", logical, page_no, offset, frame_no, physical);
