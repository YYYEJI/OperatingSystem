...
//		display the expression and result
{
	...
	while (1) {
		...
		if (strlen(expression) > 0) {
			...
			while (shm_buffer[P2C].is_loaded == TRUE) {	      // 자식 프로세스가 활동중 이므로 대기 함.
				sleep(0);
			}
			...
			while (shm_buffer[C2P].is_loaded == FALSE) {	  // 자식 프로세스로부터 결과 받기를 대기 중
				sleep(0);
			}
			strcpy(result, shm_buffer[C2P].buffer); 
			shm_buffer[C2P].is_loaded = FALSE;                // 부모 프로세스가 결과를 받았음을 의미
...
// 		send the result to the parent process by message passing
{
	...
	while (1) {
		while (shm_buffer[P2C].is_loaded == FALSE) {	      // 부모 프로세스가 표현식을 아직 전달하지 않았으므로, 대기 상태로 유지
			sleep(0);
		}
		...
		while (shm_buffer[C2P].is_loaded == TRUE) {	         //  TRUE인 동안에는 부모 프로세스가 이전 결과를 가져가지 않아 대기 상태로 유지.
			sleep(0);
		}
